...
class Spectrum:
	def __init__(self, levelsPerColor, pivotColors, lilImgDir, littleImgs, colorMap, noPics = False, skipEvery = 1):
...
def make2dSpectrum(l, spec1, spec2, lilImgDir, littleImgs, colorMap):
	...
	for i in range(0, len(spec1.pivotColors)):
		vertSpecs.append(Spectrum(l + 1, (spec1.pivotColors[i], spec2.pivotColors[i]), lilImgDir, littleImgs, colorMap, noPics = True))
		#print 'vertSpec created'
	for i in range(0, l):
		newPivotColors = []
		for v in range(0, len(vertSpecs)):
			newPivotColors.append(vertSpecs[v].colors[i])
		spec = Spectrum(spec1.levelsPerColor, newPivotColors, lilImgDir, littleImgs, colorMap)
